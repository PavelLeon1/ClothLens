...
методов эмбеддинга. В проекте есть два сегментационных режима: универсальный
SegFormer и гибридный режим, где обученная U-Net применяется к классам
`top`, `bottom`, `dress`, `outerwear`, а SegFormer остаётся для категорий,
которых нет в DeepFashion2: `shoes`, `bag`, `accessories`.
...
Фото пользователя
      |
      v
Segmentation mode router
      |
      v
U-Net или SegFormer segmentation
      |
      v
...
`Добавить изображения в каталог`. В этом режиме система сама:
...
Сегментационные backend'ы:

```text
src/clothing_search/segmentation/segformer.py
src/clothing_search/segmentation/unet_segmenter.py
```

В web-интерфейсе пользователь выбирает режим: SegFormer-only или гибридный.
Pipeline сначала определяет категорию запроса, затем выбирает подходящий
сегментатор.
...
1. `Найти похожую одежду` - загрузить query-фото, выбрать категорию и найти
   похожие товары.
2. `Добавить изображения в каталог` - загрузить один или несколько товаров в каталог без
   консоли. Для пачки файлов выбирается одна категория, а `brand`, `color`,
   `price` и `Префикс ID` можно оставить пустыми. Если `Префикс ID` пустой,
   UI создаёт `item_id` автоматически из имени файла и timestamp.
...
блоке `Добавить изображения в каталог`. Они сохраняются в `data/catalog/images`,
...
фотографии через форму `Добавить изображения в каталог`. Пользователь выбирает файлы,
...
U-Net в проекте используется как обученный backend для классов DeepFashion2.
Сейчас сделано:

- подготовлена и обучена архитектура U-Net;
...
- web-интерфейс умеет переключаться между SegFormer-only и гибридным режимом;
- лучший checkpoint U-Net достиг `val_miou = 0.7342`.

Зачем остаётся SegFormer:

- он предобучен на более широком наборе визуальных классов;
- он закрывает `shoes`, `bag`, `accessories`, которых нет в DeepFashion2;
- он служит baseline для сравнения с U-Net.

Корректная формулировка для защиты:

> В рамках практики реализована система поиска схожей одежды с модульной
> архитектурой сегментации. Для классов `top`, `bottom`, `dress`, `outerwear`
> используется обученная U-Net, а для остальных категорий применяется
> предобученный SegFormer. Такой гибридный режим позволяет сохранить широкую
> поддержку категорий и одновременно показать собственное обучение U-Net.
